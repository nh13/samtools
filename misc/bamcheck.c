...
void output_stats(stats_t *stats)
{
    ...
    printf("SN\treads unpaired:\t%lld\n", stats->nreads_unpaired);
    printf("SN\treads paired:\t%lld\n", stats->nreads_paired);
    printf("SN\treads duplicated:\t%lld\n", stats->nreads_dup);
    printf("SN\treads MQ0:\t%lld\n", stats->nreads_mq0);
    printf("SN\ttotal length:\t%lld\n", stats->total_len);
    printf("SN\tbases mapped:\t%lld\n", stats->nbases_mapped);
    printf("SN\tbases mapped (cigar):\t%lld\n", stats->nbases_mapped_cigar);
    printf("SN\tbases trimmed:\t%lld\n", stats->nbases_trimmed);
    printf("SN\tbases duplicated:\t%lld\n", stats->total_len_dup);
    printf("SN\tmismatches:\t%lld\n", stats->nmismatches);
    ...
    for (ibase=0; ibase<stats->max_len; ibase++)
    {
        ...
        {
            printf("\t%lld", stats->quals_1st[ibase*stats->nquals+iqual]);
        }
        printf("\n");
    }
    ...
    for (ibase=0; ibase<stats->max_len; ibase++)
    {
        ...
        for (iqual=0; iqual<=stats->max_qual; iqual++)
        {
            printf("\t%lld", stats->quals_2nd[ibase*stats->nquals+iqual]);
        }
        printf("\n");
    }
    if ( stats->mpc_buf )
    {
        ...
        for (ibase=0; ibase<stats->max_len; ibase++)
        {
            ...
            for (iqual=0; iqual<=stats->max_qual; iqual++)
            {
                printf("\t%lld", stats->mpc_buf[ibase*stats->nquals+iqual]);
            }
            printf("\n");
        }
    }
